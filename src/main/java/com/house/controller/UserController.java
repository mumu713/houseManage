...
public class UserController {
	...
	public Map<String,Object> login(@RequestBody LoginUser loginUser) {
		...
		if(userList.getType() == 1){
			map.put("systemRole","seniorManager");
			token = jwtUtil.createJWT(String.valueOf(userList.getId()),user.getUsername(),"seniorManager");
		}else if(userList.getType() == 2){
			map.put("systemRole","juniorManager");
			token = jwtUtil.createJWT(String.valueOf(userList.getId()),user.getUsername(),"juniorManager");
		}else{
